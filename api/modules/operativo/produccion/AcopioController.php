...
class AcopioController {
    ...
    // GET /acopios  — con filtros: estado, campaña, productor_id, certificacion
    ...
    // GET /acopios/{id}  — detalle completo con historial de kardex y análisis
    public function show(array $params): void {
        ...
        if (!$acopio) { Response::error('Acopio no encontrado', 404); return; }
        ...
        $acopio['certificaciones'] = $stmt2->fetchAll();
        ...
        $acopio['analisis'] = $stmt3->fetchAll();
        ...
        $acopio['kardex'] = $stmt5->fetchAll();

        Response::json($acopio);
    }
    ...
    public function store(): void {
        ...
        try {
            $stmt = $this->db->prepare("
                INSERT INTO acopios
                    (codigo, tipo_cafe_id, productor_id, fecha_acopio, campaña,
                     ...
                     variedad, proceso_beneficio, hora_acopio, notas)
                VALUES
            ...
            $stmt->execute([
                ...
                ':hora_acopio'         => $data['hora_acopio']          ?? null,
                ':notas'               => $data['notas']                ?? null,
            ]);
            $acopioId = Database::lastId($this->db, 'acopios');
            ...
            $stmtK = $this->db->prepare("
                ...
                VALUES
                    (:acopio_id, 'entrada', :concepto, :fecha, :cantidad_kg,
            ...
            if (!empty($data['certificaciones'])) {
                $stmtC = $this->db->prepare("
                    ...
                    VALUES (:acopio_id, :cert_id, :fi, :fv, :num)
                ");
            ...
            $this->db->commit();
            $this->show(['id' => $acopioId]);

        } catch (Exception $e) {
            $this->db->rollBack();
            Response::error('Error al crear el acopio: ' . $e->getMessage(), 500);
        }
    }
    ...
    // POST /acopios/{id}/certificaciones
    public function addCertificacion(array $params): void {
        ...
        $stmt = $this->db->prepare("
            INSERT INTO acopio_certificaciones
                (acopio_id, certificacion_id, fecha_inicio, fecha_vencimiento, numero_certificado)
            VALUES (:acopio_id, :cert_id, :fi, :fv, :num)
            ON CONFLICT (acopio_id, certificacion_id) DO UPDATE SET
    ...
    public function transformar(array $params): void {
        ...
        $stmtA = $this->db->prepare("SELECT * FROM acopios WHERE id = :id");
        $stmtA->execute([':id' => $params['id']]);
        $acopioData = $stmtA->fetch();

        if (!$acopioData) { Response::error('Acopio no encontrado', 404); return; }
        if ($data['peso_salida_kg'] > $acopioData['peso_actual_kg']) {
        ...
        try {
            ...
            $stmtT->execute([
                ...
                ':entrada'   => $acopioData['peso_actual_kg'],
            ...
            $merma = $acopioData['peso_actual_kg'] - $data['peso_salida_kg'];
            $stmtK = $this->db->prepare("
                INSERT INTO kardex
                    (acopio_id, tipo_movimiento, concepto, fecha, cantidad_kg,
                     referencia_id, referencia_tipo, usuario)
                VALUES (:acopio_id, 'transformacion', :concepto, :fecha, :kg,
            ...
            $stmtRe = $this->db->prepare("
                INSERT INTO kardex
                    (acopio_id, tipo_movimiento, concepto, fecha, cantidad_kg,
                     referencia_id, referencia_tipo, usuario)
                VALUES (:acopio_id, 'entrada', :concepto, :fecha, :kg,
                        :ref_id, 'transformacion', :usuario)
            ");
            $stmtRe->execute([
                ':acopio_id' => $params['id'],
            ...
            Response::json([
                ...
                'rendimiento_pct'   => round(($data['peso_salida_kg'] / $acopioData['peso_actual_kg']) * 100, 2),
            ]);
    ...
    private function generarCodigo(int $anio): string {
        ...
        return "ACOP-{$anio}-" . str_pad($n, 4, '0', STR_PAD_LEFT);
    }
